//Does not need a direct reference to es5-shim, just needs it to execute
//before this module does.
define(['es5-shim'], function () {
   var Narcissus;

   //NARCISSUS PARSER START
...
(function() {
    ...
    var narcissus = {
        ...
        hostSupportsEvalConst: (function() {
            try {
                return eval("(function(s) { eval(s); return x })('var x = true;')");
            } catch (e) {
                return false;
            }
        })(),
...
Narcissus.definitions = (function() {
    ...
    // Define var END, etc., based on the token names.  Also map name to index.
    var tokenIds = {};

    // Building up a string to be eval'd in different contexts.
    var consts = Narcissus.hostSupportsEvalConst ? "var " : "var ";
    for (var i = 0, j = tokens.length; i < j; i++) {
        var t = tokens[i];
        //Narcissus code likes to put extraneous commas on the end of lists,
        //which in IE6-8 creates another item in that list.
        if (!t) {
            break;
        }
        if (i > 0)
...
Narcissus.parser = (function() {
    ...
    StaticContext.prototype = {
        ...
        allow: function(type) {
            switch (type) {
              case EXPORT:
                if ((!this.inModule && !this.topLevel) || this.inFunction)
                    return false;
                // FALL THROUGH
...
//NARCISSUS PARSER END


    //Add options to the output
    Narcissus.parser.options = Narcissus.options;
